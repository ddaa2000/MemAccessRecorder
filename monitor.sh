...
cgroup_path=/home/huaziyue/cgroup-util
...
for cpu in 0; do
  mkdir ./outputs/${cpu}
  echo $passwd | sudo -S bash $cgroup_path/run_with_cgroup.sh "./build/MemAccessRecorder -b 16 -s 1000 -r 1000 -p $cpu -n 128 -m 8192 -o ./outputs/${cpu}" &
done

for cpu in 0; do
  pid_monitor=""
  while [ -z $pid_monitor ]; do
    pid_monitor=$(ps aux | grep "./build/MemAccessRecorder -b 16 -s 1000 -r 1000 -p ${cpu} -n 128 -m 8192 -o ./outputs/${cpu}" | grep -v 'grep' | grep -v 'bash' | grep -v 'cgexec' | grep -v 'sudo' | awk '{print $2}')

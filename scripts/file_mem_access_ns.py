...
def records_consumer(q, resq, gc_events, conc_threads, njt_threads):
    dicts_all = dict()
    dicts_mutator = dict()
    dicts_njt = dict()
    dicts_jt = dict()
    ...
    def update_dict(dicts, ns, addr):
        update_dict_n(dicts, windowed(ns, log_ns_window), addr, 1)
    ...
    def find_and_update(records, ns, addr):
        for record in records:
            if ns >= record['start_ms']*1e6 and ns <= record['end_ms']*1e6:
                update_dict(record['dicts'], ns, addr)
                return True
        return False
    ...
    while True:
        ...
        ns = data['timestamp'].get_nanoseconds()
        tid = int(data['tid'])
        addr = data['addr']

        # global dict
        update_dict(dicts_all, ns, addr)

        if tid in njt_threads.keys():
            update_dict(dicts_njt, ns, addr)

        # conc gc dict
        if tid in conc_threads.keys():
            if find_and_update(gc_events['conc'], ns, addr):
                continue
            find_and_update(gc_events['pause'], ns, addr) # maybe the concurrent thread also do pause works?
            # even if it is not found, it shall not be pause or mutator for the tid is not correct
            continue
            
        
        # print(ms)
        if find_and_update(gc_events['pause'], ns, addr):
            continue

        update_dict(dicts_mutator, ns, addr)
...
for i in range(0,len(cpus)*n_proc):
    for j in range(0,consumer_proc):
        proc = Process(target=records_consumer, args=(qs[i],dictq, gc_events, conc_threads, njt_threads))
        procs.append(proc)
        proc.start()
